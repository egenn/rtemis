...
options(list(rt.theme = "lightgrid", rt.fit.theme = "lightgrid"))
```
...
Command           | Input                    | Plots
----------------- | ------------------------ | ------------------------------------------------------------
`mplot3.x`        | Vector(s) `x`            | Index, histogram, density, and QQ-line plots
`mplot3.xy`       | Vectors `x` and `y`      | Scatter and/or line plots with optional fit line
`mplot3.fit`      | Vectors `x` and `y`      | `mplot3.xy` wrapper for true vs. estimated values
`mplot3.xym`      | Vectors `x` and `y`      | `mplot3.xy` scatter with marginal histogram and/or density plots
`mplot3.img`      | Matrix `x`               | 2D false color plots
`mplot3.heatmap`  | Matrix `x`               | Heatmaps with optional row and column dendrogram and colorbar
...
checkData(Sonar)
res <- resample(Sonar)
sonar.train <- Sonar[res$Resample01, ]
sonar.test <- Sonar[-res$Resample01, ]
```
