...
hytw <- function(
  ...
  if (node$depth < max.depth && nobsinnode >= minobsinnode) {
    ...
    if (!node$partlin$terminal) {
      ...
      Fval.left <- Fval +
        shrinkage * (node$partlin$part.val + node$partlin$lin.val.left)
      Fval.right <- Fval +
        shrinkage * (node$partlin$part.val + node$partlin$lin.val.right)
