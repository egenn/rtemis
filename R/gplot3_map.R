...
gplot3_map <- function(dat,
    ...
    if (labels) {
        ...
        if (length(exclude) > 0) {
            centroid_labels <- centroid_labels[!(centroid_labels$full %in%
                ...
                centroid_labels$fips %in% exclude | substr(
                centroid_labels$fips,
                1, 2
            ) %in% exclude), ]
        }
    ...
    # '- colorbar ----
    guide <- ggplot2::guides(fill = ggplot2::guide_colorbar(
        barheight = ggplot2::unit(colorbar.height, "npc"),
...
rt_gtheme_map <- function(base_size = 9, base_family = "Helvetica",
    ...
    ggplot2::theme_bw(base_size = base_size, base_family = base_family) +
        ggplot2::theme(
            axis.line = blank, axis.text = blank,
