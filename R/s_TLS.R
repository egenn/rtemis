...
#' Because there is no model, \code{predict} and other methods are not currently working with \code{s_TLS}
...
s_TLS <- function(x, y = NULL,
  ...
  # Arguments ====
  if (missing(x)) { print(args(s_TLS)); stop("x is missing") }
  if (is.null(y) & NCOL(x) < 2) { print(args(s_TLS)); stop("y is missing") }
