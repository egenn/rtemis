...
#' @param mod Character: Learner to use. Options: [learnSelect]
...
elevate1 <- function(x, y = NULL,
    ...
    if (type == "Classification") {
        ...
        names(y.train.res.aggr) <- names(y.test.res.aggr) <-
            ...
            paste0("elevate.", mod.name, ".repeat", seq(mods))
    } else {
        y.train.res.aggr <- lapply(seq(n.repeats), function(i) {
    ...
    if (save.rt) rt_save(rt, outdir, file.prefix = "elevate.", verbose = verbose)
    if (print.plot) {
