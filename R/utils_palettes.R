...
penn_heat <- function(n = 11, space = "Lab",
                      demo = FALSE,
                      colorbar = FALSE,
  ...
  if (space == "RGB") {
    penn.vlight2blue <- grDevices::colorRampPalette(
      c(color_adjust(pennCol[["blue"]], hue = -.08, val = .568), pennCol[["blue"]])
    )
    ...
    penn.blue2red2yellow.lab <- grDevices::colorRampPalette(
      c(pennCol[["blue"]], color_adjust(pennCol[["red"]], val = .1), pennCol[["yellow"]]),
      space = "Lab"
    )
    grad <- c(
  ...
  if (colorbar) {
    ...
    axis(
      side = 4, at = c(1, midpoint, n), labels = c(bar.min, bar.mid, bar.max),
      col = color_adjust("black", 0), col.axis = col.text, col.ticks = color_adjust("black", 0),
