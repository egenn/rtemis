...
# To be replaced by new unified checkData function

checkData_html <- function(x,
                           name = NULL,
                           str = FALSE,
                           recommend = TRUE,
                           reportCases.thres = NULL,
  ...
  out <- paste(out,
               paste0(strong("\n  Data types"), "________________"),
               paste("  *", strong(n.continuous), "continuous", 
                     ngettext(n.continuous, "feature", "features")),
               paste("  *", strong(n.integer), "integer", 
  ...
  out <- paste(out,
               paste0("  * ", strong(n.factor), " categorical", 
  ...
  if (n.gt2levels.nonordered > 0) {
    out <- paste(out,
                 paste("     -", strong(n.gt2levels.nonordered), 
                       "unordered categorical",
                       ngettext(n.gt2levels.nonordered, "feature has", "features have"),
                       "more than 2 levels"),
                 sep = "\n")
  ...
  if (recommend) {
    ...
    if (sum(n.character, n.constant, n.dups, n.cols.anyna, n.gt2levels.nonordered) > 0) {
      if (n.character > 0) {
        out <- paste(out,
                     paste(html_orange_bold("  * Convert the character",
                                       ngettext(n.character, "feature", "features"), "to" ,
                                       ngettext(n.character, "a factor", "factors"))),
                     sep = "\n")
      }
      ...
      if (n.constant > 0) {
        out <- paste(out,
                     paste(html_orange_bold("  * Remove the constant", ngettext(n.constant, "feature", "features"))),
                     sep = "\n")
      }
      
      if (n.dups > 0) {
        out <- paste(out,
                     paste(html_orange_bold("  * Remove the duplicated", ngettext(n.dups, "case", "cases"))),
                     sep = "\n")
      }
      
      if (n.cols.anyna > 0) {
        out <- paste(out,
                     paste(html_orange_bold("  * Consider imputing missing values or use complete cases only")),
                     sep = "\n")
      }
      ...
      out <- paste(out,
                   html_green("  * Everything looks good"),
                   sep = "\n")
    }
