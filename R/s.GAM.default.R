...
s.GAM.default <- function(x, y = NULL,
  ...
  lin.features <- if (length(lin.index) > 0) paste0(colnames(x)[lin.index], collapse = " + ") else NULL
  features <- if (is.null(lin.features)) spline.features else paste(spline.features, lin.features, sep = " + ")
  ...
  if (family$family != "multinom") {
    ...
    .formula <- lapply(c(list(paste0(y.name, " ~ ", features)),
  ...
  rt <- rtModSet(rtclass = "rtMod",
                 ...
                 parameters = list(formula = .formula,
                                   family = family,
                                   k = k,
                                   ipw = ipw,
                                   ipw.type = ipw.type,
