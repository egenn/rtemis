...
#'
#' This is the older gbm package available on CRAN. It may be preferable to use \code{s_GBM3}
...
s_GBM0 <- function(x, y = NULL,
  ...
  if (.gs) {
    ...
    .y.train <- c(.y, .y[1])
    if (verbose) msg("Training GBM on full training set...")
  }
  ...
  if (relInf) {
    ...
    mod.summary.rel <- gbm::summary.gbm(mod, plotit = plotRelInf,
  ...
  if (varImp) {
    ...
    mod.summary.perm <- gbm::summary.gbm(mod, plotit = plotVarImp,
