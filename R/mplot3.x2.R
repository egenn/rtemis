...
mplot3.x2 <- function(x,
  ...
  # [ GRID ] ====
  if (theme$grid) {
    # grid(col = theme$grid.col, lty = theme$grid.lty, lwd = theme$grid.lwd)
  ...
  if (axes) {
    ...
    # axis(1, col = theme$axes.col, col.axis = labs.col, col.ticks = tick.col, padj = x.axis.padj, tck = tck)
  ...
  #   zero.lines <- if (type == "index") TRUE else FALSE
  # }
  ...
  # if (type == "index" && theme$zerolines) {
  #   zerocol <- adjustcolor(theme$zerolines.col, theme$zerolines.alpha)
  #   if (ylim[1] <= 0 & 0 <= ylim[2]) abline(h = 0, lwd = zero.lwd, col = zerocol, lty = theme$zerolines.lty)
  #   if (xlim[1] <= 0 & 0 <= xlim[2]) abline(v = 0, lwd = zero.lwd, col = zerocol, lty = theme$zerolines.lty)
  # }
  ...
  if (type == "density" & density.mean) {
    mtext(c("Mean (SD)", lapply(seq(xl),
          ...
          col = c(theme$fg.col, unlist(col[seq_along(xl)])),
