...
cartLinBoostTV <- function(x, y = NULL,
  ...
  while (i <= max.iter) {
    ...
    mod.args <- list(x = x1, y = resid1,
                     weights = weights1,
                     save.fitted = TRUE)

    # '- Train base learner ====
    if (.learning.rate[i] != 0) {
      mods[[i]] <- do.call(learner[[(i + 1) %% 2 + 1]], args = c(mod.args,
                                                                 learner.params[[(i + 1) %% 2 + 1]]))
    } else {
      mods[[i]] <- list(fitted = Fval)
      class(mods[[i]]) <- c("nullmod", "list")
    }

    names(mods)[i] <- paste0(learner.name[[(i + 1) %% 2 + 1]], ".", i)
    ...
    if (!is.null(x.valid)) {
      # predicted.valid <- predict(mods[[i]], x.valid)
...
# predict.boost <- function(object, newdata = NULL, ...) {
#
#   if (!is.null(newdata)) {
...
#                              sapply(object$mods, function(i)
#                                predict(i, newdata) * object$learning.rate)))
#   predicted
#
# } # rtemis::predict.boost


#' Predict method for \code{cartLinBoostTV} object
#'
#' @param object \code{cartLinBoostTV} object
...
#'
#' Expand a \link{cartLinBoostTV} object by adding more iterations
#'
#' @inheritParams boost
...
#' Place model in \link{cartLinBoostTV} structure
#'
...
as.cartLinBoostTV <- function(object,
  ...
  obj <- list(mod.name = "CARTLINBOOSTTV",
              learning.rate = learning.rate,
              init = init,
              penult.fitted = NULL,
              train.ncases = NROW(x),
...
#' \pkg{rtemis} internals: Update \link{cartLinBoostTV} object's fitted values
#'
#' Calculate new fitted values for a \link{cartLinBoostTV} object.
