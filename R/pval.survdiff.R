# pval.survdiff
# Modified from survival:::print.survdiff
...
pval.survdiff <- function (x,
  ...
  if (length(x$n) == 1) {
    ...
    temp <- c(x$obs, x$exp, z, signif(pchisq(x$chisq, 1,
    ...
    #     "degrees of freedom, p=", format.pval(pchisq(x$chisq,
    #                                                  df, lower.tail = FALSE)), "\n")
  }
  list(chisq = x$chisq,
       df = df,
       p.value = pchisq(x$chisq, df, lower.tail = FALSE))
}
