...
checkData_live2 <- function(x,
    ...
    if (n.cols.anyna > 0) {
        na_feature_pct <- data.table(
            Feature = names(cols.anyna),
            Class = sapply(x[, cols.anyna], class),
            Pct_NA = sapply(seq_len(n.cols.anyna), function(i) {
                sum(is.na(x[, cols.anyna[i]])) / length(x[, cols.anyna[i]])
            })
        ) |> setorder("Pct_NA", order = -1L)

        index.incomplete <- which(!complete.cases(x))
        n.incomplete <- length(index.incomplete)
        na_case_pct <- data.table(
            Case = index.incomplete,
            Pct_NA = sapply(seq_len(n.incomplete), function(i) {
        ...
        na_feature_pct <- na_case_pct <- rep(0, n.cols)
    }
