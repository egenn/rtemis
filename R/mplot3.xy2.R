...
mplot3.xy2 <- function(x, y = NULL,
  ...
  if (all(type == "p")) {
    ...
    if (length(type) == 1) {
      ...
      if (is.null(marker.col)) {
        marker.col <- if (is.null(fit)) palette[1] else theme$fg.col
  ...
  if (group.legend) {
    ...
    if (!is.null(group.title)) group.col <- c(theme$fg.col, group.col)
  ...
  if (!is.null(myerror)) {
    if (Nxgroups == 1) {
      ...
      mtext(annot.n, fit.error.side,
            ...
            col = fit.error.col, family = theme$font.family)
      ...
      mtext(
        ...
        col = c(theme$fg.col, unlist(fit.col[seq(Nxgroups)])),
  ...
  if (rsq.pval) {
    if (is.null(rsq.col)) rsq.col <- c(theme$fg.col, unlist(adjustcolor(fit.col[seq(Nxgroups)], 2)))
